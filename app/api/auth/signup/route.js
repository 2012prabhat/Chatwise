...
export async function POST(req) {
  try {
    await connectDB();
    const { name, email, password, confirmPassword, company } = await req.json();
    ...
    const verificationToken = jwt.sign({ name, email, password, company }, process.env.EMAIL_VERIFICATION_SECRET, {
      expiresIn: "15m",
    });
