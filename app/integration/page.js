...
export default function Integration() {
  ...
  const revokeKey = async (keyId) => {
    ...
    try {
      ...
      if (response.ok) {
        setKeys((prevKeys) =>
          prevKeys.map((key) =>
            key._id === keyId ? { ...key, active: false } : key
          )
        );
      } else {
        const data = await response.json();
  ...
  const copyScript = () => {
    ...
    const script = `<script src="${baseUrl}/chatbot.js" data-api-key="add your api key here" color="" logo=""></script>`;
    navigator.clipboard
      .writeText(script)
      .then(() => {
        ...
        <div className="bg-white  rounded-3xl p-6 sm:p-10 w-full ">
          <header className="text-center mb-10">
            ...
            <p className="text-md sm:text-lg text-gray-600 font-medium">
              Generate and manage your API keys to seamlessly embed our chatbot
          ...
          <section className="mb-10">
            ...
            <p className="text-gray-700 leading-relaxed text-base">
              To integrate the Chatwise chatbot, you will need a unique API key
              for authentication. Copy a generated key and add it to your
              application&apos;s configuration to embed the chatbot widget,
              manage conversations, and retrieve user data.
            </p>
          </section>
              ...
              <ul className="divide-y divide-gray-200 -mx-6 px-6">
                {keys.length === 0 ? (
                  <p className="text-center text-gray-500 py-6">
                    No API keys found. Click the button above to generate your
                    first key!
                  </p>
                ) : (
                  keys.map((k) => {
          ...
          <section className="bg-white rounded-2xl p-6 border border-dashed border-gray-300">
            ...
            <p className="text-gray-700 mb-4">
              Copy the script below and paste it anywhere in your HTML file
              before the closing <code>&lt;/body&gt;</code> tag. Make sure to
              replace <code>YOUR_API_KEY</code> with your generated key.
            </p>
            <div className="relative">
              <pre className="bg-gray-100 p-4 rounded-xl font-mono text-sm overflow-x-auto">
                {`<script src="${baseUrl}/chatbot.js" data-api-key="add your api key here" color="" logo=""></script>`}
              </pre>
              <button
                onClick={copyScript}
